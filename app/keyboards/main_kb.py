...
def get_voice_bots_keyboard() -> InlineKeyboardMarkup:
    ...
    kb.row(InlineKeyboardButton(text="⚖️ Юриспруденция", callback_data="voice_jurisprudence"))
...
def get_text_bots_keyboard() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="⚖️ Юридическая компания", callback_data="text_jurisprudence"))
    ...
    kb.row(InlineKeyboardButton(text="🎁 Бесплатная демка", callback_data="text_demo"))
...
def get_cases_keyboard() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="🤖 ИИ-маркетолог", url="https://t.me/marketingairabbit_bot"))
    return kb.as_markup()
...
def get_contact_keyboard(category: str = None) -> InlineKeyboardMarkup:
    ...
    # Выбираем URL в зависимости от категории
    if category in ["voice", "text"]:
        contact_url = "https://t.me/airabbitsaler_bot"
    else:
        contact_url = "https://t.me/maximdeveloper"
        
    kb.row(InlineKeyboardButton(text="📱 Связаться с менеджером", url=contact_url))
    kb.row(InlineKeyboardButton(text="◀️ Назад", callback_data="back_to_main"))
    return kb.as_markup() 