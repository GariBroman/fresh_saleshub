...
async def process_text_demo(callback: CallbackQuery, state: FSMContext):
    ...
    demo_text = """Создадим нейросо-сотрудника вашей компании. Вы пройдете квалификацию и наш бот приведет вас к целевому действию. Для создания демо-версии нам понадобиться ваш сайт.

Отправляет сайт по ссылке в лм и мы отправим вам готовый результат в ближайшее время"""
